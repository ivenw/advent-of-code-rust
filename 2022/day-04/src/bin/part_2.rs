...
fn function(input: String) -> u32 {
    input
        ...
        .map(|line| {
            ...
            let (first_elf_set, second_elf_set) = (
                (first_elf_start..=first_elf_end).collect::<HashSet<u32>>(),
                (second_elf_start..=second_elf_end).collect::<HashSet<u32>>(),
            );

            if first_elf_set.intersection(&second_elf_set).count() > 0 {
                return 1;
            }
